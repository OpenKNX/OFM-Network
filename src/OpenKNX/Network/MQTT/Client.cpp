    ...
    namespace Network
    {
        namespace MQTT
        {
            ...
            void Client::loop()
            {
                switch (_state)
                {
                    ...
                    case STATE_READY:
                        ...
                        if (delayCheck(_lastActivity, (uint32_t)_keepAlive * 1500 + 2000))
                        {
            ...
            void Client::handlePublish(const Packet &pkt)
            {
                ...
                {
                    if (topicMatches(sub.topic.c_str(), topic))
                        sub.callback(topic, p, payloadLen);
                }
            }
            ...
            bool Client::tcpConnect(const char *host, uint16_t port)
            {
                ...
                if (!c->connect(host, port))
                    return true; // async — loop() will detect
                _state = STATE_CONNECTED;
                sendConnect();
                return true;
            }
