...
namespace OpenKNX
{
    namespace Format
    {
        namespace JSON
        {
            ...
            // Zahlentoken NUL-terminiert nach buf — der Reader kann über einen
            // rohen, nicht terminierten Puffer laufen.
            static bool copyNumberToken(const char* p, const char* end, char* buf, size_t bufSize)
            {
                const char* tokEnd = skipNumber(p, end);
                size_t len = (size_t)(tokEnd - p);
            ...
            // Invariante: konsumiert immer mindestens ein Zeichen (sonst dreht sich
            // findKey()/findIndex() bei kaputtem JSON endlos).
            static const char* skipValue(const char* p, const char* end)
            {
                ...
                if (*p != '{' && *p != '[')
                {
                    const char* after = skipNumber(p, end);
                    return (after > p) ? after : p + 1;
                }
            ...
            static const char* findKey(const char* p, const char* end, const char* key)
            {
                while (p < end)
                {
                    ...
                    p = skipValue(p, end);
                    if (p <= before)
                        return nullptr; // kein Fortschritt → malformed
                    p = skipWhitespace(p, end);
                    if (p < end && *p == ',')
                        ++p;
                }
            ...
            int32_t Reader::toInt(int32_t def) const
            {
                ...
                if (!isNumber())
                    return def;
                char tok[32];
                if (!copyNumberToken(_start, _end, tok, sizeof(tok)))
                    return def;
                char* endp = nullptr;
                long val   = strtol(tok, &endp, 10);
                return (endp && endp > tok) ? (int32_t)val : def;
            }
            ...
            uint32_t Reader::toUInt(uint32_t def) const
            {
                ...
                return (endp && endp > tok) ? (uint32_t)val : def;
            }
            ...
            float Reader::toFloat(float def) const
            {
                ...
                if (!copyNumberToken(_start, _end, tok, sizeof(tok)))
                    return def;
                char* endp = nullptr;
                float val  = strtof(tok, &endp);
                return (endp && endp > tok) ? val : def;
            }
