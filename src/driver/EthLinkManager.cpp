...
void EthLinkManager::tick(bool carrierUp, bool established)
{
    ...
    else // no working (IP) connection at this stage yet
    {
        ...
        {
            // An AutoIP build gets a longer window (see ETH_STAGE_TIMEOUT_MS) instead of no escalation at
            // all: waiting for AutoIP must not step down the speed, but a link that produces neither DHCP
            // nor AutoIP in that time is dead and has to.
